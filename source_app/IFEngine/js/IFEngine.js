class IFEngine{
		...
		this.menu = {
			principale: {
				...
				opzioni: {
					...
					3: {
						label: i18n.IFEngine.menu.delete,
						callback: async () => {
	...
	async restore() {
		...
		do{
			let savedGames = await this.getSavedGames()
	...
	async delete() {
		...
		let answer = await this.yesNoQuestion(i18n.IFEngine.questions.areYouSureQuestion);
		
		if(answer){
			for (let sg of Object.keys(savedGames)){
				this.storage.removeItem(sg)
			}
			await this.CRT.println(i18n.Thesaurus.defaultMessages.done, {nlBefore: 1, nlAfter: 1})
		}
	...
	async getSavedGames(){
		...
		if(Object.keys(savedGames).length == 0){
			await this.CRT.printTyping(i18n.IFEngine.warnings.noData, {nlBefore:1 , nlAfter: 1});
			return false;	
		}
